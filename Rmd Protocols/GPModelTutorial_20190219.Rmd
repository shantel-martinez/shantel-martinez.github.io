...
#Data Organization
accuracy80 = as.data.frame(matrix(cv_results6)) 
...
`accuracy80 = as.data.frame(matrix(cv_results6))` the matrix `cv_results6` is made into a dataframe called `aaccuracy80`    
`accuracy80$model<-"rrBLUP"` add in a new column `model` into the df and fill the column with the characters `rrBLUP`  
`names(accuracy80) <- c("PA",  "model")` rename the column names to `PA` and `model`         
`accuracyrr<-accuracy80` rename the df to `accuracyrr`    
...
#Data Organization
accuracy80 = as.data.frame(matrix(cv_results5)) 
...
#Data Organization
accuracy80 = as.data.frame(matrix(cv_results7)) 
